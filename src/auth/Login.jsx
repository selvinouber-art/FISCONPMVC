...
export default function Login({ onLogin }) {
  ...
  async function handleLogin(e) {
    ...
    if (!mat || !senha) { setErro('Preencha matrícula e senha.'); return }
    setCarregando(true)
    try {
      const resultado = await rpc('autenticar_usuario', { p_matricula: mat, p_senha: senha })
      if (!resultado || !resultado.success) { setErro('Matrícula ou senha incorretos.'); return }
    ...
    <div className="login-fundo">
      <div className="login-card">
        <div style={{ textAlign: 'center', marginBottom: '28px' }}>
          ...
          />
          <h1 style={{ fontSize: '2.4rem', color: '#1A56DB', letterSpacing: '0.1em', margin: 0 }}>FISCON</h1>
          <p style={{ fontSize: '0.85rem', color: '#475569', margin: '4px 0 2px' }}>Fiscalização de Obras e Posturas</p>
          <p style={{ fontSize: '0.75rem', color: '#94A3B8', margin: 0 }}>Prefeitura Municipal de Vitória da Conquista — BA</p>
        </div>

        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <label style={{ fontSize: '0.85rem', fontWeight: '600', color: '#374151' }}>Matrícula</label>
            <MascaraInput tipo="matricula" value={matricula} onChange={setMatricula} autoComplete="username" />
          </div>
